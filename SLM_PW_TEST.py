...
def calibration():
    ...
    lenspaper = Image.open("/Users/anthonylu/Library/CloudStorage/GoogleDrive-AnthonyLu@lbl.gov/.shortcut-targets-by-id/1VJBVeRRN_5zVF1Gqm0fEKW9dfVeRScci/SLM/feedbackAlgorithm/test50/crosshairImg.png")
    # display(lenspaper)
    ...
    lenspaper = zoom_at(lenspaper, width/2 - 33, height/2 + 117, 1)
...
def feedback(initial, testno, count, threshold = 175, blur = 5, innerBlur = 15, range = 5, maxIter = 100, yshift = 4, plot = False):
    global aboveMultArray, belowMultArray, totalMultArray, totalMultImg
    ...
    initialImg = ImageOps.flip(ImageOps.mirror(initialImg))
    
    # initialImg = zoom_at(initialImg, width/2 - 151, height/2 + 15, 1)     # Test 51
    initialImg = zoom_at(initialImg, width/2 - 33, height/2 + 117, 1)
    initialImg = initialImg.rotate(1.2)
    ...
    #####
    # Use the initial beam image to create the "goal" or "target" image that the final beam should look like
    #####
    global xi, yi, goalImg, goalArray
    ...
    # if count == 0:
    #     belowMultArray = np.zeros(aboveMultArray.shape)
    #     xi, yi = (initialArray >= int(threshold)).nonzero()     # Retrieve x and y positions of every pixel with a value greater than the threshold
    #     goalArray = initialArray.copy()
    #     goalArray[xi,yi] = int(threshold)
    #     goalImg = Image.fromarray(goalArray)
    # else:
    #     x, y = (initialArray >= int(threshold)).nonzero()
    #     xdiff = np.setdiff1d(x, xi)
    #     ydiff = np.setdiff1d(y, yi)
    #     xi = np.append(xi, xdiff)
    #     yi = np.append(yi, ydiff)
    #     goalArray[xi,yi] = int(threshold)
    #     goalImg = Image.fromarray(goalArray)
    
    #####
    # This is just a test of making the goal image without changing every trial. Seeing if edge imperfections go away (test 9)
    #####
    
    # # if count == 0:
    # goalArray2 = goalArray
    # goalArray = initialArray.copy()
    # goalArray = np.where(goalArray > 150, 150, goalArray)
    # goalImg = Image.fromarray(goalArray)
    global stacked, stacked2, xi, yi, x, y
    
    if count == 0:
        xi, yi = (initialArray >= int(threshold-5)).nonzero()
        # xi, yi = (np.any((initialArray >= int(threshold-5))|(initialArray )))
        stacked = np.stack((xi, yi), axis=-1)
        # print(xi, yi)
        # print(len(stacked))
    else:
    ...
    #     try:
    #####
    # Take difference between initial image and goal image to find which pixels are brighter and which are dimmer
    #####
    ...
    xa, ya = (aboveImgArray > 0).nonzero()
    xb, yb, = (belowImgArray < 0).nonzero()
    aboveImgArray[xb, yb] = 0
    belowImgArray[xa, ya] = 0
    belowImgArray = np.abs(belowImgArray)
    # print("\nAbove\nMax: " + str(np.amax(aboveImgArray)) + "\nMin: " + str(np.amin(aboveImgArray)))
    # print("\nBelow\nMax: " + str(np.amax(belowImgArray)) + "\nMin: " + str(np.amin(belowImgArray)))
    aboveImgArray = np.abs(np.int8(aboveImgArray))
    ...
    aboveMultArray = np.multiply(aboveMaxBlurredArray, blazedData)
    aboveMultArray = np.multiply(aboveMultArray, aboveBlurredArray)
    # aboveMultArray = np.multiply(aboveBlurredArray, blazedData)
    
    ##################
    # belowMultArray2 = belowMultArray     # Save previous multArray as multArray2 to take average later     ##### BELOWMULTARRAY DOES NOT INITIALLY EXIST AND NEITHER DOES ABOVEMULTARRAY: FIX WHEN RECURSION
    ...
    aboveMultArray = (aboveMultArray * np.amax(aboveBlurredArray) / (np.amax(aboveMultArray))).astype(np.int32) # np.uint8     # Normalize multiplied grating to the max of the subtracted image
    ...
    #####
    # Find difference between 150 and max of result image and turn into multiplication of above and below. Used to add and subtract by smaller increments as image gets closer to goal
    #####
    ...
    # totalMultArray = (totalMultArray2 + 0.5 * aboveMultArray - 0.5 * belowMultArray).astype(np.int32)
    totalMultArray = (totalMultArray2 + aboveMultArray - belowMultArray).astype(np.int32)
    ...
    # yshiftArray[xi,yi] = 80 - totalMultArray[xi,yi] * 2
    # yshiftArray[xi,yi] = 30
    # yshiftArray[xi,yi] = 70 - (totalMultArray[xi,yi] **2) / 100

    
    yshiftArray = gaussian_filter(yshiftArray, sigma = 15)
    totalMultArray = totalMultArray + yshiftArray
    ...
    # totalMultImg.save("/Users/anthonylu/Library/CloudStorage/GoogleDrive-AnthonyLu@lbl.gov/.shortcut-targets-by-id/1VJBVeRRN_5zVF1Gqm0fEKW9dfVeRScci/SLM/feedbackAlgorithm/test" + str(testno) + "/" + str(count+1) + "WFTEST_15outside.png")
    # displayt(aboveImg, 'aboveImg')
    ...
    # array1 = initialArray
    # array2 = aboveImgArray
    
    # array3 = (0.5*array1 + 1.5*array2) / 2
    ...
    if plot:
        ...
        # plt.plot(np.arange(width), diffImgStrip, label = "DiffImg")
        ...
        plt.xlim(750,1125)
        plt.ylim(0,260)
        plt.title("Test " + str(testno) + ", Trial " + str(count) + ", AveDiff = " + str(np.round(diff,2)) + ", WFTEST_15outside")
        # plt.savefig("/Users/anthonylu/Library/CloudStorage/GoogleDrive-AnthonyLu@lbl.gov/.shortcut-targets-by-id/1VJBVeRRN_5zVF1Gqm0fEKW9dfVeRScci/SLM/feedbackAlgorithm/test" + str(testno) + "/plot" + str(count) + "WFTEST_15outside.png")
        plt.show()
        # """
...
def runFeedback(testno):
    ...
    for i in np.arange(1):
        if i == 0:
            feedback("feedbackAlgorithm/test" + str(testno) + "/initial", testno = testno, count = i,
                      plot = True
                     )
        else:
            try:
                ...
                feedback("feedbackAlgorithm/test" + str(testno) + "/" + str(i) + "Result", testno = testno, count = i,
                          plot = True
                         )
            except:
                # count = i - 1
...
# for i in np.arange(39, 46):
#     runFeedback(i)
    
# count = 0
# feedback("feedbackAlgorithm/test35/initial", plot = True)
# count = 1
# feedback("feedbackAlgorithm/test35/1Result", plot = True)
# count = 2
# # feedback("feedbackAlgorithm/test35/2Result", plot = True)
# count = 3
# # feedback("feedbackAlgorithm/test35/3Result", plot = True)
# count = 4
# # feedback("feedbackAlgorithm/test35/4Result", plot = True)
# count = 5
# # feedback("feedbackAlgorithm/test35/5Result", plot = True)
# count = 6
# # feedback("feedbackAlgorithm/test35/6Result", plot = True)
# count = 7
# # feedback("feedbackAlgorithm/test35/7Result", plot = True)
# count = 8
# # feedback("feedbackAlgorithm/test35/8Result", plot = True)
# count = 9
# # feedback("feedbackAlgorithm/test35/9Result", plot = True)
# count = 10
# # feedback("feedbackAlgorithm/test35/10Result", plot = True)

# aboveMultArray = np.zeros(aboveMultArray.shape)
# belowMultArray = np.zeros(belowMultArray.shape)
# totalMultArray = np.zeros(totalMultArray.shape)
...
# Test 51: Just some test gratings to fix wavefront
