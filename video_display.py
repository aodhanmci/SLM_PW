...
class Page(tk.Frame):
    ...
    def update(self):
        ...
        if self.lineout_iso_toggle:
            try:
                self.cx, self.cy, self.dx, self.dy, self.phi = lbs.beam_size(self.CCD_array)
                self.CCD_array_major = lbs.major_axis_arrays(self.CCD_array, self.cx, self.cy, self.dx, self.dy, self.phi)
                self.CCD_array_minor = lbs.minor_axis_arrays(self.CCD_array, self.cx, self.cy, self.dx, self.dy, self.phi)
