...
img = Image.open("C:\\Users\\loasis\\Documents\\GitHub\\SLM_PW\\settings\\PreSets\\HAMAMATSU_black.png")
img = ImageOps.grayscale(img)
imgMap = img.load()
...
# initial = Image.open("/Users/anthonylu/Documents/GitHub/SLM_PW/testCGH.png")
# print(np.asarray(initial).shape)
...
# retrievedPhase = GS_Python("/Users/anthonylu/Documents/GitHub/SLM_PW/gaussianRectangle.png", 
                            # 10)

# Image.fromarray(retrievedPhase).show()
...
def makeRectangle():
    # image = Image.open("/Users/anthonylu/Documents/LBNL/SLM/black.png")
    ...
    # width, height = img.size
    image = Image.new(mode="L", size=(1280,1024))
    ...
    # mf = ImageFont.truetype('/Users/anthonylu/Documents/LBNL/SLM/Roboto-Medium.ttf', 30)
    mf = ImageFont.truetype('/Users/anthonylu/Documents/GitHub/SLM_PW/Roboto-Medium.ttf', 30)
    draw.text((width/3-30, height/3-40), str(1), font=mf, fill = 255*255*255)
    draw.text((2*width/3-30, height/3-40), str(2), font=mf, fill = 255*255*255)
    draw.text((width/3-30, 2*height/3-40), str(3), font=mf, fill = 255*255*255)
    draw.text((2*width/3-30, 2*height/3-40), str(4), font=mf, fill = 255*255*255)
            
    
            # # 9 DOTS
            # if width/3-3 <= i <= width/3+3 and height/3-3 <= j <= height/3+3 or 1*width/3-3 <= i <= 1*width/3+3 and 2*height/3-3 <= j <= 2*height/3+3 or 2*width/3-3 <= i <= 2*width/3+3 and 1*height/3-3 <= j <= 1*height/3+3 or 2*width/3-3 <= i <= 2*width/3+3 and 2*height/3-3 <= j <= 2*height/3+3:
            #     imageMap[i,j] = 255
    
    imageArray = asarray(image)
    # plt.plot(np.arange(width), imageArray[int(height/3-2),:], label = "SLM Grating", color = "C2")
    # plt.xlim((0,10))
    # plt.ylim((0,255))
    # plt.show()
    
    display(image)
    # image.save("/Users/anthonylu/Library/CloudStorage/GoogleDrive-AnthonyLu@lbl.gov/.shortcut-targets-by-id/1VJBVeRRN_5zVF1Gqm0fEKW9dfVeRScci/SLM/feedbackAlgorithm/HAMAMATSU_255_rectangle_vertical_lines.png")
    image.save("/Users/anthonylu/Documents/GitHub/SLM_PW/calibration/HAMAMATSU/crosshairNums.png")
...
### HAMAMATSU
# 12.5 micron pixel pitch
# want 23.205 micron width
    
"""

def newNewBlazed(n, amp):
    image = Image.new(mode="L", size=(1280,1024))
    width, height = image.size
    imageMap = image.load()
    count = 0
    ...
    plt.plot(np.arange(width), imageArray[int(height/2),:], label = "SLM Grating", color = "C2")
    # plt.xlim((0,101))
    plt.ylim((0,255))
    plt.show()

    image.save("C:\\Users\\loasis\\Documents\\GitHub\\SLM_PW\\GratingTests\\" + str(n) + ' ' + str(amp) + ".png")
    # image.show()
